...
#include "Common/StdOutStream.h"			// ’Ç‰Á
...
#include "../../UI/Common/ExtractingFilePath.h"			// •ÏX

#include "../../UI/Common/ArchiveExtractCallback.h"		// •ÏX
    ...
    UInt64 packSize)
{
  ...
  if (g_StdOut.GetProgressDialog())
  {
    ...
    arc->Archive->GetArchiveProperty(kpidSolid, &prop);
    g_StdOut.GetProgressDialog()->SetArchiveInfo((prop.vt != VT_EMPTY) ? VARIANT_BOOLToBool(prop.boolVal) : false, (prop.vt != VT_EMPTY) ? ARCHIVETYPE_7Z : ARCHIVETYPE_ZIP);
  }
  /* ’Ç‰Á‚±‚±‚Ü‚Å */
}
...
STDMETHODIMP CArchiveExtractCallback::GetStream(UInt32 index, ISequentialOutStream **outStream, Int32 askExtractMode)
{
  ...
  *outStream = 0;
  ...
  if (askExtractMode == NArchive::NExtract::NAskMode::kExtract && !_testMode)
  {
    ...
    if (!_isSplit)
    {
    if (g_StdOut.GetProgressDialog() && _curSizeDefined) g_StdOut.GetProgressDialog()->SeekNextFile(_curSize);	// ’Ç‰Á
    ...
    if (g_StdOut.GetProgressDialog())
    {
      ...
      UInt64 nCompFileSize = (prop.vt != VT_EMPTY) ? ConvertPropVariantToUInt64(prop) : 0;
      prop.vt = VT_EMPTY;
...
//  RINOK(_extractCallback2->SetOperationResult(operationResult, _encrypted));		// íœ
  /* ’Ç‰Á ‚±‚±‚©‚ç */
  HRESULT res = _extractCallback2->SetOperationResult(operationResult, _encrypted);
